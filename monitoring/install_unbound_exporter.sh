...
mkdir -p /opt/unbound_exporter
cd /opt/unbound_exporter

echo "=== Salvando o fonte do unbound_exporter ==="
cat > unbound_exporter.go << 'EOF'
package exporter
...
echo "=== Compilando unbound_exporter ==="
go build -o unbound_exporter unbound_exporter.go

echo "=== Instalando binário ==="
mv unbound_exporter /usr/local/bin/
...
echo "=== Testando ==="
sleep 2
curl -s http://localhost:9167/metrics | head || true
